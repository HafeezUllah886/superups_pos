        ...
        <form method="post" id="form_submit" action="{{ route('customer.store') }}">
            ...
            <div class="row">
                <div class="col-md-2 col-2">
                    ...
                    <select required name="product_id[]" id="1product_id" onchange="price_search('1')" class="form-control select2">
          ...
          <div class="col-md-2 col-2">
                        ...
                        <input name="date" type="date" value={{ \Carbon\Carbon::now() }} class="form-control">
                        @error('date')
                            <span class="text-danger">
                                {{ $message }}
                            </span>
 ...
 function add_row() {
    ...
    $("#form").append(
        '<div id="row'+line_no+'" class="row">' +
                ' <div class="col-md-2">' +
                    '<label for="product_id">Product</label>'+
                    '<select required id="'+line_no+'product_id" name="product_id[]" onchange="price_search('+line_no+')" class="form-control  select2">'+
                        '<option value="">Select Product</option>'+
                        '@foreach ($products as $item)'+
                        '<option {{ old("product_id")==$item->id ? "selected" : "" }} value="{{ $item->id }}">' +
                            '{{ $item->name }}'+
